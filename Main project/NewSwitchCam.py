...
items = "Web Cam", "Phone Cam","Browse Image"
...
def destBrowse():
    destDirectory=filedialog.askdirectory(initialdir="")
...
com3.current(0)
com3.place(x=10, y=10, width=80)
button3 =Button(win,text='switch',command=choose)
button3.place(x=100,y=10)
bt3=Button(win,text="Capture",bg="Green",fg="white",command=capture,font=("Times New Roman",20),padx=3,pady=3)
bt3.place(x=350, y=530, width=100) 
browsebtn=Button(win,text="BROWSE",command=destBrowse)
browsebtn.place(x=10,y=580,width=100)
#saveEntry=Entry(win,width=20,textvariable=destPath)
#saveEntry.place(x=436,y=5)

#bt4=Button(win,text="Browse",fg="Black",bg="White",command=destBrowse)
#bt4.place(x=60,y=500)
imgs = "rgb", "gray"
switch = ttk.Combobox(win,values=imgs)
switch.place(x=700, y=10, width=80)
switch.current(0)
...
def show():
    ...
    if switch.get()=="lab":
        imageFrame(lab,label,5,2,820,620)
    if switch.get()=="hsv":
        imageFrame(hsv,label,5,2,820,620)
    if switch.get()=="hls":
        imageFrame(hls,label,5,2,820,620)
    if switch.get()=="canny":
        imageFrame(canny,label,5,2,820,620)
    label.after(5,show)
